...
describe("initialView", () => {
  it("opens normal images at fit, centered", () => {
    expect(initialView(4000, 3000, 800, 600)).toEqual({
      zoom: 0.2,
      x: 0,
      y: 0,
    });
...
describe("readout", () => {
  ...
  it("shows live percent and signed offsets otherwise", () => {
    expect(readout({ zoom: 2.13, x: 512.4, y: -273.6 }, 0.2)).toBe(
      "213% · +512,-274",
    );
  });
});
...
describe("outOfWhack", () => {
  ...
  it("is false at the centered initial view", () => {
    expect(outOfWhack({ zoom: 0.5, x: 0, y: 0 }, 1600, 1200, 800, 600)).toBe(
      false,
    );
  });

  it("is false when zoomed far in but still centered", () => {
    expect(outOfWhack({ zoom: 8, x: 0, y: 0 }, 1600, 1200, 800, 600)).toBe(
      false,
    );
  });
  ...
  it("is true once most of the image is dragged out of the window", () => {
    ...
    expect(outOfWhack({ zoom: 0.5, x: 700, y: 0 }, 1600, 1200, 800, 600)).toBe(
      true,
    );
  });

  it("is true when the image is entirely outside the window", () => {
    expect(outOfWhack({ zoom: 0.5, x: 5000, y: 0 }, 1600, 1200, 800, 600)).toBe(
      true,
    );
  });
