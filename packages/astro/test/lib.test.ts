...
describe("readingTime", () => {
  ...
  it("ceils by 200 wpm", () => {
    expect(readingTime(Array(401).fill("w").join(" "))).toBe(3);
  });
});

describe("slugifyCategory", () => {
  it("matches WP category slugs", () => {
    expect(slugifyCategory("Home Automation Projects")).toBe(
      "home-automation-projects",
    );

    expect(slugifyCategory("MSOE Undergraduate Projects")).toBe(
      "msoe-undergraduate-projects",
    );
  });
});
...
describe("internalHrefKey", () => {
  const ORIGINS = [
    "https://www.half-built-robots.com",
    "https://half-built-robots.com",
  ];
  ...
  it("the site's own absolute origins reduce to their path", () => {
    expect(
      internalHrefKey("https://www.half-built-robots.com/about/", ORIGINS),
    ).toBe("/about/");
...
describe("formatPostDate", () => {
  it("threads a locale parameter (proves it is not welded to en-US)", () => {
    expect(formatPostDate(new Date(Date.UTC(2026, 0, 5)), "de-DE")).toContain(
      "Januar",
    );
  });
});
...
describe("archivePagePath", () => {
  it("page 1 is the site root", () => {
    expect(archivePagePath(1)).toBe("/");
  });
  ...
  it("clamps below 1 to the root", () => {
    expect(archivePagePath(0)).toBe("/");
  });

  it("prefixes a base for category archives", () => {
    expect(archivePagePath(1, "/category/robots")).toBe("/category/robots/");

    expect(archivePagePath(2, "/category/robots")).toBe(
      "/category/robots/page/2/",
    );
  });
});
...
describe("publication ordering (audit A1/A2 root)", () => {
  it("byNewest breaks same-day ties by published timestamp", async () => {
    ...
    const mkPost = (
      date: string,
      published?: string,
    ): import("../src/lib/ordering").Publishable => ({
      data: {
        date: new Date(date),
    ...
    expect(
      [early, dateOnly, late].sort(byNewest).map((p) => +publishedAt(p)),
    ).toEqual([
...
describe("assertPageRoutable", () => {
  ...
  it("accepts ordinary top-level and resolved-parent pages", () => {
    expect(() => {
      assertPageRoutable(page("about"), undefined);
    }).not.toThrow();

    expect(() => {
      assertPageRoutable(page("privacy-policy", "policies"), page("policies"));
    }).not.toThrow();
  });

  it("rejects a parent reference that resolves to nothing", () => {
    expect(() => {
      assertPageRoutable(page("privacy-policy", "policeis"), undefined);
    }).toThrow(/matches no page slug/);
  });
...
describe("visiblePosts", () => {
  ...
  it("drops drafts by default", () => {
    expect(visiblePosts(all, false).map((p) => p.data.slug)).toEqual([
      "a",
      "c",
    ]);
  });
...
describe("forwardLinkVisible", () => {
  ...
  it("throws on a slug no post has, so a typo cannot pass as a draft", () => {
    expect(() => forwardLinkVisible("typo", posts, false)).toThrow(
      /no post has slug "typo"/,
    );
  });
});
...
describe("resolvePostHref", () => {
  const posts = [
    { data: { slug: "live", date: new Date("2026-09-01T00:00:00Z") } },
  ];
  ...
  it("throws on an unknown slug", () => {
    expect(() => resolvePostHref("typo", posts)).toThrow(
      /no post has slug "typo"/,
    );
  });
});
